...
def main():
    ap = argparse.ArgumentParser(
        description="CPU-only CoRot-LRDSUN autoregressive rollout -> VTU/PVD for ParaView"
    )
    ...
    try:
        torch.set_num_interop_threads(max(1, min(4, int(args.threads))))
    except RuntimeError:
        pass
    ...
    print("CoRot-LRDSUN CPU rollout -> VTU", flush=True)
